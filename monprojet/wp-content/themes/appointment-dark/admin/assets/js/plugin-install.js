jQuery(document).ready(function($) {
    'use strict';
    var this_obj = appointment_dark_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'appointment_dark_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.webriti-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'appointment_dark_hide_customizer_notice';
            sdata.appointment_dark_plugin_slug = slug;
        }else{
            sdata.action = 'appointment_dark_hide_customizer_companion_notice';
        }
